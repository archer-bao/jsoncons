...
Returns `true` when there are no more JSON texts to be read from the stream, `false` otherwise
    ...
    void read()
Reads the next JSON text from the stream and reports JSON events to a [json_input_handler](json_input_handler), such as a [json_decoder](classes/json_decoder.md).
